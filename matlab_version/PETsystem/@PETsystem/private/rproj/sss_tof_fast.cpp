...
/*
	TOF-based SSS: created by Jian Zhou
	Date: Sept. 2014
*/
...
/*
    ...
    if (nrhs < 9) {
        ...
        if (prhs[8] == NULL) {
            ...
            tbin_max = INT(mxGetScalar(prhs[8]));
        }
    }
*/
    INT* tbin_list = 0;
    std::vector<INT> tbin;
    if (nrhs < 9) {
        mexErrMsgTxt("not supply tbin_list!");
        return;
    } else {
        if (prhs[8] == NULL || (mxGetClassID(prhs[8]) != mxINT32_CLASS)) {
            mexErrMsgTxt("invalid tbin, should be an array of int32!");
            return;
        } else {
            tbin_list=(INT*)mxGetPr(prhs[8]);
            INT nb = mxGetM(prhs[8]) * mxGetN(prhs[8]);
            tbin.resize(nb);
            for (INT i=0; i < nb; i ++) { 
                tbin[i] = tbin_list[i];
                //mexPrintf("%d\n", tbin[i]);
...
// read energy window and resolution
    ...
    REAL* block_info = 0;
    if (nrhs < 11) {
        mexErrMsgTxt("not specify detector module infor");
        return;
    } else {
        block_info = mxGetPr(prhs[10]);
        if (block_info == NULL) {
            mexErrMsgTxt("invalid block info!");
            return;
        }
        if (mxGetM(prhs[10]) * mxGetN(prhs[10]) < 2) {
            mexErrMsgTxt("invalid block info, must be "
                         "[num_of_trans_crystals_per_block, num_of_trans_blocks]");
            return;
        } 
    }
    INT K=INT(block_info[0]);
    INT num_of_blocks=INT(block_info[1]);


#if 1
    mexPrintf("Single Scatter Simulator:\n"
              "  Image size: %d,%d,%d\n"
              "  Voxel size: %g,%g,%g\n"
              "  Number of crystals on a ring: %d (%d,%d)\n"
              "  Number of rings: %d\n"
              "  Number of LORs (per plane): %d\n"
              "  TOF info: %g, %g (ps), number of bins: %d\n"
              "  Energy window: [%g, %g] (kev), resolution: %g%%\n",
              ni, nj, nk, voxsize[0], voxsize[1], voxsize[2],
              num_of_xtals, K, num_of_blocks, 
              nring, num_of_xps, tw_resolution, 
              tw_spacing, tbin.size(), 
              energy_window_min, energy_window_max, energy_resolution*100);
#endif
    ...
    // need to design
    //
    mexPrintf("Selecting scatter points ...\n");    
    REAL threshold = 1.0;
    ...
    std::vector<std::vector<std::vector<REAL> > > geom(nring); // it works better 
    ...
    for (INT rid = 0; rid < nring; rid ++) {
        ...
        for (n = 0; n < num_of_xtals; n ++) {
            ...
            // test only
            geom[rid][n].resize(scatter_point_buffer.size());
            ...
            for (INT m = 0; m < scatter_point_buffer.size(); m ++) {
                ...
                REAL wy = sy - xtal_y;
                REAL wz = sz - xtal_z;
                geom[rid][n][m] = fabs(wx * bx + wy * by) / sqrt(wx*wx+wy*wy+wz*wz);
    ...
    plhs[0] = mxCreateDoubleMatrix(num_of_xps * nbin, 1, mxREAL);
    REAL* S = mxGetPr(plhs[0]);
    
    mexPrintf("OK! ready to go ...\n");
...
//                }
                ...
                for (INT m = 0; m < scatter_point_buffer.size(); m ++) { //scatter_point_buffer.size()
                    ...
                    if (E2 < ENERGY_MIN ||
                        E2 > ENERGY_MAX) { 
                        // the energy of scattered photon has exceeded our interpolation table [100 800], so discard it!
                        // in theory it won't happen 'cause scattering causes the loss of energy so the scattered photon
                        // always has energy no greater than 511.0 kev 
                        continue;
                    }
...
#else // soft cut, compute probability within the energy window
                    ...
                    // nearest interpolation based on lookup table
                    ...
                    REAL g1 = geom[r1][xp.id1-1][m];
                    REAL g2 = geom[r2][xp.id2-1][m];

                    REAL* e1_buf = new REAL[nbin];
                    REAL* e2_buf = new REAL[nbin];

                    REAL t = sqrt(len2_sq / len1_sq);
                    REAL vBx = sx + t * v1x;
                    REAL vBy = sy + t * v1y;
                    REAL vBz = sz + t * v1z;
                    raytracer.fproj_TOF_SSS_AS(xtal1_x, xtal1_y, xtal1_z,
                                               sx, sy, sz, 
                                               vBx, vBy, vBz, 
                                               emis_image, tbin, e1_buf);
                    REAL inv_t = 1.0 / t;
                    REAL vAx = sx - inv_t * v2x;
                    REAL vAy = sy - inv_t * v2y;
                    REAL vAz = sz - inv_t * v2z;
                    raytracer.fproj_TOF_SSS_SB(vAx, vAy, vAz,
                                               sx, sy, sz,
                                               xtal2_x, xtal2_y, xtal2_z, 
                                               emis_image, tbin, e2_buf);

                    // put together, ignore xtal efficiency (or assume identical to all)
                    // note: 1/(len1_sq*len2_sq) accounts for the solid angle effect
                    // if g1 and g2 are used, g1/len1_sq * g2/len2_sq accounts for the solid angle effect
                    // where g1 and g2 are cosines of the incident angle of the photons 
#if 1
                    REAL ccc = 1.0 / (len1_sq*len2_sq) * mu * KN * pb * g1 * g2;   
#else
                    REAL ccc = 1.0 / (len1_sq*len2_sq) * mu * KN * pb;   
#endif
                    REAL ex1 = exp(-(a1 + ratio*a2));
                    REAL ex2 = exp(-(a2 + ratio*a1));                  
                    for (INT tt = 0; tt < nbin; tt ++) {
                        REAL I = ex1 * e1_buf[tt] + ex2 * e2_buf[tt]; 
                        S[offset + tt] += ccc * I;
                    }
