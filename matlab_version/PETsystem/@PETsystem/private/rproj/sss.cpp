...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    if (nrhs < 8) {
        ...
        energy_info = mxGetPr(prhs[7]);
        if (energy_info == NULL) {
            mexErrMsgTxt("invalid energy info!");
            return;
        }

        if (mxGetM(prhs[7]) * mxGetN(prhs[7]) < 3) {
...
#if USE_TOF
    mexErrMsgTxt("currently not support TOF!");
    return;
#endif
      
    // create a raytracer    
    ImageRayTracer raytracer(ni, nj, nk, voxsize[0], voxsize[1], voxsize[2]);
    REAL vk = voxsize[2];
    REAL vj = voxsize[1];
    REAL vi = voxsize[0];
    ...
    std::vector<std::vector<std::vector<REAL> > > emis_ray_sum(nring);
    std::vector<std::vector<std::vector<REAL> > > attn_ray_sum(nring);    
    for (INT rid = 0; rid < nring; rid ++) {
        
        emis_ray_sum[rid].resize(num_of_xps);
        ...
        for (n = 0; n < num_of_xtals; n ++) {
            ...
            emis_ray_sum[rid][n].resize(scatter_point_buffer.size());
            ...
            for (INT m = 0; m < scatter_point_buffer.size(); m ++) {
                ...
                emis_ray_sum[rid][n][m] = 
                    raytracer.fproj(sx, sy, sz, xtal_x, xtal_y, xtal_z, emis_image);
                attn_ray_sum[rid][n][m] = 
                    raytracer.fproj(sx, sy, sz, xtal_x, xtal_y, xtal_z, attn_map);
...
//                S[n] += raytracer.fproj(xtal1_x, xtal1_y, xtal1_z, 
//                                        xtal2_x, xtal2_y, xtal2_z, emis_image) / d12;
                ...
                for (INT m = 0; m < scatter_point_buffer.size(); m ++) {
                    ...
                    REAL pb = 0.5 * (1 - erf((energy_window_min - E2)/(511.0 * energy_sigma))) - 
                              0.5 * (1 - erf((energy_window_max - E2)/(511.0 * energy_sigma)));
#endif
                    ...
                    REAL ratio = g_lut_energy_to_attn[index] / 0.0096; // 0.009687 is too big!
