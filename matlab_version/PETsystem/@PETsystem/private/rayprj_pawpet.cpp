...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    if (nrhs < 6) {
        ...
        if (mxGetM(prhs[5]) != 7) {
            mexErrMsgTxt("invalid listmode format, must be 7 x num_of_prompts!");
            return;
        }
    }
    ...
    // read max step
    REAL* rotation_angle_list = 0;
    if (nrhs < 7) {
        mexErrMsgTxt("not specify rotation angles!");
        return;
    } else {
        rotation_angle_list = mxGetPr(prhs[6]);
        if (rotation_angle_list == NULL) {
            mexErrMsgTxt("not specify rotation angles!");
        }
    }
    INT num_of_angles = mxGetM(prhs[6]) * mxGetN(prhs[6]);
    std::vector<REAL> cos_t(num_of_angles);
    std::vector<REAL> sin_t(num_of_angles);
    for (int n = 0; n < num_of_angles; n ++) {
        cos_t[n] = cos(rotation_angle_list[n] * ImageRayTracer::PI / 180.0);
        sin_t[n] = sin(rotation_angle_list[n] * ImageRayTracer::PI / 180.0);
    }
    ...
    mexPrintf("\tImage size: %d,%d,%d\n"
              ...
              "\tNumber of rings: %d\n"
              "\tNumber of rotation angles: %d\n"
              "\tNumber of prompts: %d\n",
              ni, nj, nk, voxsize[0], voxsize[1], voxsize[2],
              num_of_xtals, num_of_dois, nring, num_of_angles, num_of_prompts);
#endif
    ...
    for (n = 0; n < num_of_prompts; n ++) {
        ...
        REAL ct = cos_t[e.step_id];
        REAL st = sin_t[e.step_id];
            
        REAL x1_rot = ct * x1 - st * y1;
        REAL y1_rot = st * x1 + ct * y1;
        REAL x2_rot = ct * x2 - st * y2;
        REAL y2_rot = st * x2 + ct * y2;

        proj[n] = raytracer.fproj(x1_rot, y1_rot,
