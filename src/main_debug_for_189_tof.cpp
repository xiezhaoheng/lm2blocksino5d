...
#define N_AX_BLK 56 // 14 ax blocks per unit * 8 units
...
#define N_TOF 94 //UIH defined [15,17,19,23,25,27], we use 27 to maintain compatibility with old format
...
int main(int argc, char* argv[]) {
	...
	//cout << "Starting " <<plut[i].nv <<" ..."<<plut[i].nu<< " ..." <<i<< endl;  //Print transaxial ID (zhoujian's       'scanner.getDefaultSinogramCrystalPairs' format)
	...
	cout << "Start with main loop "<< endl;
	// main loop
        int read_count_outof_traxial_tof=1; //This is because of zhoujian's  SinogramCrystalPairs has larger(22) accepted BLK Difference,which covers smaller FOV in transaxial plane, and TOF FOV is different
        int read_count_total=1; //
	Lm* plm = new Lm; // list-mode event
	unsigned int* psino_blk = new unsigned int[SIZEOF_SINO*N_AX_BLK*N_AX_BLK*N_TOF]; // block TOF sino
	while (!feof(pfile_in)) {
		...
		for (int i = 0; i < read_count; i++) {
			...
			int TOF_AB_temp = plm->tof;
                        if (TOF_AB_temp<0 && TOF_AB_temp>-95){
			 TOF_AB = TOF_AB_temp;}
                        else{
                        continue; }
                        // 
			...
			int axBiB = plm->axIDB / (N_AX_CRYS_PER_BLK);
                        ...
                        //cout << "Reading Listmode "<<"A "<<txBiA<<"B "<<txBiB<< endl;
                       //cout << "Reading Listmode "<<"A "<<txBiA<<"B "<<txBiB<<"TOF "<<TOF_AB<< endl;
			...
			int ind_blk_sino = idx_tx_blk + SIZEOF_SINO * axBiB
				+ SIZEOF_SINO * N_AX_BLK * axBiA+ SIZEOF_SINO * N_AX_BLK * N_AX_BLK*(TOF_AB+94); // 5-D sinogram 
			...
			if (ind_blk_sino == -1||idx_tx_blk==(SIZEOF_SINO-1)||(TOF_AB+94)<0||(TOF_AB+94)>188) { // provides fault toleranceW
                       //ind_blk_sino == -1 represents crystal-based UIH listmode out-of-bound zhoujian's sinogram,
                       //idx_tx_blk==SIZEOF_SINO-1 represents BLK-based listmodeID covered by zhoujian's sinogram, but some of crystal UIH listmode out-of-bound because of integer division in C is equivalent to 'floor', which make the one last entry of sinogram has very high value .
           ...
           //cout << "This lm file includes: " << read_count<< " events, which has"<< read_count_outof_traxial_tof<< " out of our transaxial FOV"<< endl;

	}

	printf("So far totol event is %d, %d out of our transaxial FOV\n", read_count_total,read_count_outof_traxial_tof);
	// write to file
	cout << "Save sino... "<< endl;
	fwrite(psino_blk, sizeof(unsigned int), SIZEOF_SINO*N_AX_BLK*N_AX_BLK*N_TOF, pfile_out);
